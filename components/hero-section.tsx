...
export function HeroSection({ data, onUpdate, isEditing = false }: HeroSectionProps) {
  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-between overflow-hidden border-b border-[#E5E5E5] bg-white">
      {/* Top spacing */}
      <div className="pt-40 md:pt-48 px-8 md:px-20 lg:px-32 flex-1 flex flex-col justify-start">
        {/* Subtle tagline */}
        <FadeUp delay={0.1}>
          <div className="flex items-center gap-4 mb-10 md:mb-14">
            <span className="inline-block w-1.5 h-1.5 rounded-full bg-[#000000]" />
            <EditableText
              isEditing={isEditing}
              value={heroData.tagline}
              onSave={(v) => handleUpdate("tagline", v)}
              as="span"
              className="text-[10px] font-bold tracking-[0.4em] text-black/40 uppercase"
            />
          </div>
        </FadeUp>
        ...
        <MouseParallax intensity={10} className="max-w-6xl">
          <EditableText
            ...
            as="h1"
            className="text-[clamp(3rem,12vw,10rem)] font-bold tracking-tighter leading-[0.85] text-black uppercase block w-full outline-none"
          />
        </MouseParallax>

        {/* Description and CTA with heavy spacing */}
        <FadeUp delay={0.4} className="mt-16 md:mt-24 flex flex-col md:flex-row items-start md:items-end gap-10 md:gap-24 w-full text-left">
          <EditableText
            isEditing={isEditing}
            value={heroData.description}
            onSave={(v) => handleUpdate("description", v)}
            as="p"
            className="text-lg md:text-xl text-black/50 max-w-sm leading-relaxed font-light block"
          />
          <div className="flex items-center gap-6">
            <MagneticButton strength={0.2}>
              <Link
                href="#work"
                className="inline-flex items-center gap-3 bg-black text-white text-[10px] font-black tracking-widest px-10 py-5 rounded-lg hover:bg-black/80 transition-all active:scale-95 group"
              >
            ...
            <MagneticButton strength={0.2}>
              <Link
                href="#contact"
                className="inline-flex items-center gap-2 text-[10px] font-black tracking-widest text-black border border-black/10 px-10 py-5 rounded-lg hover:border-black transition-all active:scale-95"
              >
      ...
      {/* Modern Stats bar - Minimal & Light borders */}
      <div className="border-t border-[#E5E5E5] grid grid-cols-2 md:grid-cols-4 bg-white">
        {heroData.stats.map((stat, i) => (
          <FadeUp key={stat.label + i} delay={0.6 + i * 0.1}>
            <div
              className={`px-12 py-10 md:py-14 flex flex-col gap-2 ${
                i < heroData.stats.length - 1 ? "border-r border-[#E5E5E5]" : ""
              }`}
            >
              <EditableText
                isEditing={isEditing}
                ...
                as="span"
                className="text-4xl md:text-5xl font-bold tracking-tighter text-black block"
              />
              <EditableText
                isEditing={isEditing}
                value={stat.label}
                ...
                as="span"
                className="text-[9px] font-black tracking-[0.3em] text-black/30 uppercase block"
              />
            </div>
          </FadeUp>
        ))}
