...
function ServiceCard({ service, index }: { service: any; index: number }) {
  ...
  return (
      <motion.div
          ...
          style={{ marginTop: index === 0 ? 0 : `-${index * 20}px`, zIndex: index + 1 }}
      >
        <div>
        ...
        <div className="mt-10">
          ...
          <div className="flex items-start gap-6 max-w-[750px]">
            <div className="relative w-14 h-14 border border-white/20 rounded-full flex items-center justify-center bg-[#1a1a1a]">
              <motion.div animate={{ x: eyePos.x, y: eyePos.y }} transition={{ type: "spring", stiffness: 250, damping: 25 }} className="w-5 h-5 bg-white rounded-full shadow-[0_0_15px_white]" />
            </div>
            <div className="min-h-[100px]">
              <AnimatePresence mode="wait">
                <motion.p key={activeTab} initial={{ opacity: 0, x: 15 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -15 }} className="text-[#999] text-[1.4vw] leading-relaxed font-medium">
