...
// Chargement du JSON
  ...
  return (
      <section className="w-full bg-[#F8F8F8] px-16 py-32 font-haas overflow-hidden">

        {/* LABEL WORK */}
        <motion.span
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 0.5, y: 0 }}
            className="block text-black text-[32px] mb-12 font-bold"
        >
          Work
        </motion.span>
          ...
          <motion.h2 variants={container} initial="hidden" whileInView="visible" className="text-[3vw] leading-[1.1] text-black font-bold tracking-tighter flex flex-wrap">
            {words.map((word, i) => (
                <motion.span variants={child as any} key={i} className="mr-[0.25em] inline-block">
                  {word.replace(",", "") === "diligent" ? (
                      <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#888] to-[#333]">{word}</span>
                  ) : word}
                </motion.span>
            ))}
          </motion.h2>

          <motion.h2 variants={container} initial="hidden" whileInView="visible" custom={1.2} className="text-[3vw] leading-[1.1] text-black font-bold tracking-tighter flex flex-wrap">
            {words2.map((word, i) => (
      ...
      >
        <div className="relative aspect-[16/10] overflow-hidden rounded-[30px] bg-[#EEE] z-10">
          <motion.img
              ...
              className={`w-full h-full object-cover transition-all duration-700 ${!project.isLive && isHovered ? 'grayscale-[0.8] blur-[2px]' : ''}`}
          />



          {/* TITRE AVEC FLÈCHE QUI POUSSE (Collision Effect) */}
          <div className="absolute bottom-10 left-10 z-20 flex items-center">

            {/* LA FLÈCHE DEVANT QUI POUSSE */}
            <motion.div
            ...
            <div className="flex">
              {titleLetters.map((letter: string, i: number) => (
                  <motion.span
                      ...
                      className="text-white text-[3.5vw] font-bold leading-none tracking-tighter inline-block whitespace-pre"
                  >
                    {letter}
                  </motion.span>
              ))}
